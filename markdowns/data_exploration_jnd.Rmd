...
VegDataRaw <- read_excel(paste0(here::here(),
                                "/data/Veg data - validated.xlsx"), sheet = "Plants") %>%
  mutate(SiteID = case_when(SiteName == "Westerhall" ~ "WH",
                            SiteName == "Conference" ~ "CN",
                            SiteName == "Levera" ~ "LV",
                            SiteName == "Mt. Hartman" ~ "MH"),
         SiteID = paste0(SiteID, "-", "T", Transect, "-",  "P", Plot )) %>%
  left_join(read_excel(paste0(here::here(),
                                "/data/Veg data - validated.xlsx"), sheet = "Site Joins"))%>%
...
BirdData <- VegDataRaw %>% select(SiteName, SiteID, Independent)%>%
  right_join(read_csv(paste0(here::here(), "/data/bird_data_independent reps.csv"))%>%
  rename(SiteName = "Site")%>%
  mutate(SiteID = paste0(Code, "-", Tnum, "-", Pnum))) %>%
...
BirdDatDateSum <- VegDataRaw %>% select(SiteName, SiteID, Independent)%>%
  ...
  rename(SiteName = "Site")%>%
  mutate(SiteID = paste0(Code, "-", Tnum, "-", Pnum))) %>%
  group_by(SiteName, SiteID, Independent, Date,Visit, Species) %>%
  summarise(Abundance = (sum(Abundance, na.rm = TRUE)) %>% round())%>%
  pivot_wider(names_from  = "Species",
              values_from = "Abundance",
              values_fill = 0)%>% filter(!is.na(SiteName))
