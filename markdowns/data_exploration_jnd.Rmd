...
png(paste0(here::here(), "/figures/PlantBirdCongruenceMantelProcustes.png"),
    units = "in",
...
NMDSPlotsBirds(NMDSList = BirdsSiteNMS,
                 ...
                 DataX =  BirdDataOG,
                 Choices = c(1:3),
                 Groups = c("Species", "Food", "Behaviour",  "Nesting", "Habitat","Type"),
                 Type = "Site")
NMDSPlotsBirds(NMDSList = BirdsSiteNMS,
                 FileNames = "Abundance-OG",
                 DataX =  BirdDataOG,
...
```{r}
png(paste0(here::here(), "/figures/StableIsotopes.png"),
...
```{r MammalsOcupancyandPrep, include=FALSE}
...
dataMammalsGrid <- 
  ...
  left_join(dataMammals %>%  mutate(Species = ifelse(str_detect(Species, "Rat|rat"), "Brown Rat", Species)) %>%
   ...
   mutate(across(where(is.numeric), ~ ifelse(replace_na(., 0)>0,1,0)))
...
```{r PlotDataMammals, include=FALSE}
png(paste0(here::here(), "/figures/MammalDetections.png"),
      width = 8, height = 5, res = 600, units = "in", bg = "white")
 dataMammals %>%  mutate(Species = ifelse(str_detect(Species, "Rat|rat"), "Brown Rat", Species)) %>%
    group_by(Species)%>%  tally()%>% mutate(prop = n/sum(n))%>%
  ggplot(aes( y = prop, x = Species, label = n))+
  geom_bar(position = "dodge", stat = "identity")+
    geom_text(vjust = -0.25)+
     theme_gaea()+
  scale_y_continuous(labels = scales::percent)+
  labs(shape = "", color = "", fill = " ", y = "Proportion of Detections", x = "")
  dev.off()
