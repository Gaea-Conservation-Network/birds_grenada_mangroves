...
png(paste0(here::here(), "/outputs/PlantBirdCongruenceMantel.png"),
    units = "in",
    res = 600, width = 8, height = 8)
TreeHeightBirdID %>% filter( Group %in% c("Overall"))%>% mutate(Type = "Tree Height")%>%
...
bind_rows(BasalAreaBirdID %>% filter( Group %in% c("Overall"))%>% mutate(Type = "Basal Area")) %>%
 bind_rows(CanopyWidthBirdID %>% filter(Group %in% c("Overall"))%>% mutate(Type = "Canopy Width"))%>%
  mutate(Significance = case_when(`P-Value`<0.001 ~ "***",
                                  ...
                                  TRUE ~ ""))%>%
#filter(Test == "Mantel")%>%
  mutate(Test = ifelse(Test == "Mantel", "Mantel Test", "Procustes Analysis"))%>%
  ggplot(aes(x = Type, y = Strength, fill = Group))+
  geom_bar(stat = "identity", position = "dodge")+
  scale_fill_brewer(palette = "BrBG")+
  geom_text(aes(label = Significance), size = 6, position = position_dodge(width = .9))+
    theme_gaea()+
  facet_grid(~Test)+
  labs( y = expression(paste("Strength of Cross-Taxon Congruence (r)/", m^2, ")")),
        x = " ", fill = "",
        caption = "Birds & Plants")+
  theme(legend.position = "none")
dev.off()
...
We can use `dbFD` in the `FD` package to create these metrics, using a matrix of species abundances and binary traits.
...
```{r test bird and plants, echo=FALSE}

PBData <- 
  tibble(SiteName = BirdDataOG %>% ungroup() %>% 
                       arrange(SiteID)%>% select(SiteName) %>% reduce(c) ,
                 SiteType = BirdDataOG %>% 
           ungroup() %>% arrange(SiteID)%>% select(SiteType)%>% reduce(c)  , 
                 SiteID = str_remove_all(BirdDataOG %>% ungroup() %>% arrange(SiteID)%>% 
               select(SiteID)%>% reduce(c), "-0")  ,
                 ...
                 FunctionalEveness = BirdFD$FEve,
                 FunctionalDivergence = BirdFD$FDiv,
         Richness = rowSums(BirdDataOG %>% ungroup() %>% arrange(SiteID)%>% select(ANCH:ZEND) %>%
                              mutate(across(ANCH:ZEND, ~ifelse(.x>=1,1,0)), )))%>%
            left_join(VegMatrixSiteID)


summary(lm(FunctionalDispersion ~ BlackCanopyWidth + RedCanopyWidth + WhiteCanopyWidth,
   data = PBData))

summary(lm(FunctionalRichness ~ BlackCanopyWidth + RedCanopyWidth + WhiteCanopyWidth,
   data = PBData))

summary(lm(FunctionalDivergence ~ BlackCanopyWidth + RedCanopyWidth + WhiteCanopyWidth,
   data = PBData))

summary(lm(FunctionalRichness ~ BlackCanopyWidth + RedCanopyWidth + WhiteCanopyWidth,
   data = PBData))

summary(lm(Richness ~ BlackCanopyWidth + RedCanopyWidth + WhiteCanopyWidth,
   data = PBData))
```
